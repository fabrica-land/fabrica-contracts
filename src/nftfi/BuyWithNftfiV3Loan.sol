...
import {IBuyWithNftfiV3Loan} from "./IBuyWithNftfiV3Loan.sol";
import {INftfiV3LoanCoordinator} from "./INftfiV3LoanCoordinator.sol";
import {INftfiV3LoanOffer} from "./INftfiV3LoanOffer.sol";
import {INftfiHub} from "./INftfiHub.sol";

contract BuyWithNftfiV3Loan is IBuyWithNftfiV3Loan, IERC165, ERC165, IERC721Receiver, IERC1155Receiver, ICallee, DydxFlashloanBase {

    ISoloMargin public immutable DYDX_SOLO_MARGIN;
    INftfiV3LoanOffer public immutable NFTFI_V3_LOAN_OFFER;
    INftfiV3LoanCoordinator public immutable NFTFI_V3_LOAN_COORDINATOR;
    ...
    struct CallParams {
        ...
        address receiver;
        Order order;
        INftfiV3LoanOffer.Offer offer;
        INftfiV3LoanOffer.Signature signature;
    }
    ...
    constructor(
        ...
        NFTFI_V3_LOAN_OFFER = INftfiV3LoanOffer(nftfiLoanAddress);
        INftfiHub nftfiHub = INftfiHub(NFTFI_V3_LOAN_OFFER.hub());
        NFTFI_V3_LOAN_COORDINATOR = INftfiV3LoanCoordinator(nftfiHub.getContract(NFTFI_V3_LOAN_OFFER.LOAN_COORDINATOR()));
    ...
    function callFunction(
        ...
        address escrowAddress = INftfiV3LoanOffer(NFTFI_V3_LOAN_OFFER).getEscrowAddress(address(this));
        IERC1155(params.offer.nftCollateralContract).setApprovalForAll(escrowAddress, true);
        uint32 loanId = NFTFI_V3_LOAN_OFFER.acceptOffer(
            params.offer,
            params.signature
        );
        // 7) Mint the obligation receipt
        NFTFI_V3_LOAN_OFFER.mintObligationReceipt(loanId);
        // 8) Transfer the obligation receipt to the receiver
        INftfiV3LoanCoordinator.Loan memory loan = NFTFI_V3_LOAN_COORDINATOR.getLoanData(loanId);
        IERC721(NFTFI_V3_LOAN_COORDINATOR.obligationReceiptToken()).transferFrom(
            address(this),
            params.receiver,
            loan.smartNftId
        );
